...
import {
  ...
  ChevronDown, X, Store, ShoppingBag,
} from 'lucide-react';
      ...
      <div className="bg-white rounded-[20px] p-6 w-[500px] shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[12.5px] text-[#64748b] mt-0.5">Thêm shop mới vào hệ thống quản lý</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-[8px] hover:bg-[#f1f5f9] text-[#94a3b8]"><X size={16}/></button>
        </div>
      ...
      {errorShops.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 mb-5 bg-[#fef2f2] border border-[#fecaca] rounded-[12px]">
          ...
          <span className="text-[13px] text-[#dc2626] font-medium">
            {errorShops.length} shop lỗi kết nối: {errorShops.map(s => s.name).join(', ')}
          </span>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#f0f3f8]">
                  {['Tên shop', 'Kênh', 'Trạng thái', 'Sản phẩm', 'Listing', 'Doanh thu', 'Đơn hàng', 'Sync cuối', 'Ngày KN', ''].map((h, i) => (
              ...
              <tbody>
                {shops.map(s => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-[#f8fafc] hover:bg-[#f8fafc] transition-colors">
                      ...
                      <td className="px-3 py-3 text-[12px] text-[#64748b]">{fmtDate(s.connected_at)}</td>
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-0.5">
                          ...
                          <button title="Xoá shop" onClick={() => handleDelete(s.id)} disabled={deletingId === s.id}
                ...
                {shops.length === 0 && (
                  <tr><td colSpan={10} className="px-3 py-10 text-center text-[13px] text-[#94a3b8]">Không tìm thấy shop nào</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {view === 'grid' && (
          <div className="grid grid-cols-3 gap-3">
            {shops.map(s => {
              const Icon = STATUS_ICON[s.status] ?? CheckCircle;
              return (
                <div key={s.id} className="border border-[#e8edf5] rounded-[14px] p-4 hover:shadow-md hover:border-[#cbd5e1] transition-all">
